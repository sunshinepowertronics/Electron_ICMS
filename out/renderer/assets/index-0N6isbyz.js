...
function createWriteEchoWaiter(expectedPrefix, timeoutMs) {
  ...
  const promise = new Promise((resolve, reject) => {
    ...
    off = window.icms.onSerialData((payload) => {
      ...
      if (!bytes) return;
      const slave = expectedPrefix[0] & 255;
      const func = expectedPrefix[1] & 255;
      if (bytes.length === 5 && bytes[0] === slave && bytes[1] === (func | 128)) {
        const ex = bytes[2] ?? 0;
        finishErr(new Error(`Device rejected write (Modbus exception 0x${ex.toString(16).toUpperCase()})`));
        return;
      }
      if (bytes.length !== 8) return;
      for (let i = 0; i < expectedPrefix.length; i++) {
        if ((bytes[i] & 255) !== (expectedPrefix[i] & 255)) {
          return;
        }
      }
      finishOk();
...
async function sendWriteAndWaitEcho(req, timeoutMs, meta) {
  ...
  let lastError = "Timed out waiting for device write echo";
...
async function writeSettingsHoldingUInt16(slaveId, spec, valueU16) {
  ...
  if (!f) return { ok: false, error: "Could not build write register frame" };
  return sendWriteAndWaitEcho([...f], 6e3);
}
async function writeSettingsAsciiRegisters(slaveId, spec, text, onProgress) {
  ...
  for (let r = 0; r < numRegs; r++) {
    ...
    if (!f) return { ok: false, error: "Could not build write register frame" };
    const req = [...f];
    const res = await sendWriteAndWaitEcho(req, 6e3);
