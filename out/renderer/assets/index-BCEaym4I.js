    ...
    versions: [
      {
        ...
        sidebar_params: [
          {
            option_name: "Monitor",
            query1: "slave_id 04 00 00 00 35 crc16MOdbus",
            query2: "slave_id 02 00 00 00 18 crc16MOdbus",
            query_mapping: {
              cooling_params: "query1",
              power_params: "query1",
...
function resolveMonitorEntry(modelName, firmwareVersion) {
  ...
  const fwCandidates = Array.from(/* @__PURE__ */ new Set([fw, fw.replace(/^v/i, "").trim()]));
  const { products: products2 } = productsData;
  const product = products2.find((p) => p.product_name.trim().toLowerCase() === modelNorm);
  if (!product) return null;
  const versionEntry = product.versions.find(
    (v) => fwCandidates.some((cand) => v.version.trim().toLowerCase() === cand.toLowerCase())
  );
...
function Traffic({ connected, path, baudRate, slaveId, lines, onClear }) {
...
function isFailStatus(out, keys) {
  for (const k of keys) {
    const v = out[k]?.value;
...
function forceSensorFailDisplay(out) {
  const failCell = { value: "---", color: "#f87171" };
...
function Dashboard() {
  const { view, serialConnected, serialLines, serialPath, serialBaudRate, serialSlaveId, clearSerialTraffic } = useDisplayView();
...
function appendTrafficTx(bytes, label) {
  const text = `[TX ${label}] ${toHex(bytes)}`;
  window.dispatchEvent(new CustomEvent("icms:traffic-line", { detail: text }));
}
function appendTrafficTxFailed(label, error) {
  window.dispatchEvent(new CustomEvent("icms:traffic-line", { detail: `[TX ${label} failed] ${error}` }));
}
...
async function sendWriteAndWaitEcho(req, timeoutMs, meta) {
...
async function sendWriteWithOptionalEcho(req, timeoutMs, meta) {
...
async function writeSettingsAlarmBool(slaveId, query, spec, on) {
  ...
  if (query === "query2") {
    ...
    return sendWriteAndWaitEcho([...f2], 2500, logTx);
  }
...
async function writeEics141DateTimeSync(slaveId, _controllerDateSpec, _timeSpec, now2 = /* @__PURE__ */ new Date()) {
  const target = new Date(now2.getTime() + 5e3);
  ...
  const frame = [
    ...
    97,
    target.getDate() & 255,
    target.getMonth() + 1 & 255,
    target.getFullYear() % 100 & 255,
    target.getHours() & 255,
    target.getMinutes() & 255,
    target.getSeconds() & 255
  ];
      ...
      children: /* @__PURE__ */ jsxRuntimeExports.jsxs(
        "div",
        {
          ...
          children: [
            ...
            success ? /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "settings-edit-success", role: "status", children: success }) : null,
...
const LOG_LIMITS = [10, 50, 100, 500, 1e3];
const RECEIVE_WINDOW_MS = {
  10: 3e3,
  50: 5e3,
  100: 7e3,
  500: 1e4,
...
const ALARM_BYTE_1_BITS = ["FAN1 INT FL", "FAN2 INT FL", "FAN1 EXT FL", "FAN2 EXT FL", "RSVD", "TCAB FAIL", "TAMB FAIL", "HRT"];
const ALARM_BYTE_2_BITS = ["LCT", "USYS HG", "USYS LW", "DOOR", "SMOKE", "RSVD", "FAN1 INT STATUS", "FAN2 INT STATUS"];
const ALARM_BYTE_3_BITS = ["FAN1 EXT STATUS", "FAN2 EXT STATUS", "MACHINE STATUS", "RSVD", "SPARE", "SPARE", "SPARE", "SPARE"];
...
function splitCsvCells(text) {
  const cells = text.split(",").map((c) => c.trim().replace(/^\.+/, ""));
...
function filterUsefulBits(bits) {
  const names = [];
  const indexes = [];
...
function parseLogsTable(text) {
  ...
  const isDateToken = (v) => /^\d{1,2}\/\d{1,2}\/\d{2,4}$/.test(v.trim());
  const rows = [];
  if (dateColIdx >= 0) {
    const startIndexes = [];
...
function Logs() {
  ...
  const [limit, setLimit] = reactExports.useState(LOG_LIMITS[0]);
  const [busy, setBusy] = reactExports.useState(false);
  const [error, setError] = reactExports.useState(null);
  const [txHex, setTxHex] = reactExports.useState(null);
  const [rxAscii, setRxAscii] = reactExports.useState("");
  const [busyDots, setBusyDots] = reactExports.useState("");
  const parsedTable = reactExports.useMemo(() => parseLogsTable(rxAscii), [rxAscii]);
  const reqIdRef = reactExports.useRef(0);
  const unsubscribeRef = reactExports.useRef(null);
  const rxBufferRef = reactExports.useRef("");
  const lastRxAtRef = reactExports.useRef(0);
  reactExports.useEffect(() => {
    return () => {
  ...
  const handleGetLogs = async () => {
    ...
    const count = Math.max(0, Math.min(65535, Number(limit) || 0)) & 65535;
    const slave = parseSlaveByte(serialSlaveId);
    const hi = count >> 8 & 255;
    const lo = count & 255;
    const body = [slave, 133, 0, 1, hi, lo];
    const crc = crc16Modbus(new Uint8Array(body));
    const frameArr = [...body, crc & 255, crc >> 8 & 255];
    ...
    try {
      ...
      const listenMs = RECEIVE_WINDOW_MS[limit] ?? 2e3;
      await new Promise((resolve) => window.setTimeout(resolve, listenMs));
      if (reqIdRef.current !== reqId) return;
      const extendDeadline = Date.now() + (RECEIVE_EXTEND_MAX_MS[limit] ?? 5e3);
      while (reqIdRef.current === reqId && Date.now() < extendDeadline) {
        ...
        await new Promise((resolve) => window.setTimeout(resolve, RECEIVE_POLL_MS));
      }
      if (reqIdRef.current !== reqId) return;
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs(
      "div",
      {
        ...
        children: [
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { display: "flex", alignItems: "center", gap: "0.6rem", marginLeft: "auto" }, children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx(
              "label",
              {
            ...
            /* @__PURE__ */ jsxRuntimeExports.jsx(
              "select",
              {
                ...
                children: LOG_LIMITS.map((n) => /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: n, children: n }, n))
              }
            ),
            /* @__PURE__ */ jsxRuntimeExports.jsx(
              "button",
              {
                ...
                children: busy ? "Getting…" : "Get Logs"
              }
            )
          ] })
        ]
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs("section", { className: "serial-readout", "aria-label": "Logs response", style: { marginTop: "1rem" }, children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "serial-readout-bar", children: /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "serial-readout-summary", children: error ? /* @__PURE__ */ jsxRuntimeExports.jsx("strong", { children: error }) : txHex && busy ? /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
        "Request sent - receiving data",
        busyDots
      ] }) : txHex ? /* @__PURE__ */ jsxRuntimeExports.jsx(jsxRuntimeExports.Fragment, { children: "Receive window completed." }) : /* @__PURE__ */ jsxRuntimeExports.jsx(jsxRuntimeExports.Fragment, { children: "Ready." }) }) }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "serial-readout-log", role: "log", "aria-live": "polite", children: [
        txHex ? /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "serial-readout-line", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { style: { opacity: 0.75 }, children: "[TX]" }),
          " ",
          txHex
        ] }) : null,
        rxAscii && parsedTable?.rows.length ? /* @__PURE__ */ jsxRuntimeExports.jsx(
          "div",
          {
            style: {
              overflowX: "auto",
            ...
            children: /* @__PURE__ */ jsxRuntimeExports.jsxs("table", { style: { width: "100%", height: "100%", borderCollapse: "separate", borderSpacing: 0, fontSize: "0.82rem" }, children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("thead", { children: /* @__PURE__ */ jsxRuntimeExports.jsxs("tr", { children: [
                ...
                parsedTable.headers.map((h, idx) => /* @__PURE__ */ jsxRuntimeExports.jsx(
                  "th",
                  {
                    style: {
                      textAlign: "center",
              ...
              /* @__PURE__ */ jsxRuntimeExports.jsx("tbody", { children: parsedTable.rows.map((row, rowIdx) => /* @__PURE__ */ jsxRuntimeExports.jsxs(
                "tr",
                {
                  style: {
                  ...
                  children: [
                    ...
                    parsedTable.headers.map((_, colIdx) => /* @__PURE__ */ jsxRuntimeExports.jsx(
                      "td",
                      {
                        style: {
                          padding: "0.48rem 0.65rem",
                          ...
                          whiteSpace: "nowrap",
                          textAlign: "center",
                          color: ALARM_BIT_COLUMN_SET.has(parsedTable.headers[colIdx]) && (row[colIdx] ?? "") === "1" ? "#ff5c5c" : ALARM_BIT_COLUMN_SET.has(parsedTable.headers[colIdx]) && (row[colIdx] ?? "") === "0" ? "#58d68d" : "#d0d7de"
                        },
                        children: row[colIdx] ?? ""
                      },
                      `cell-${rowIdx}-${colIdx}`
        ...
        ) : rxAscii ? /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "serial-readout-line", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { style: { opacity: 0.75 }, children: "[RX ASCII]" }),
          " ",
          rxAscii
        ] }) : txHex && !busy && !error ? /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "serial-readout-placeholder", children: "No valid logs response frame received." }) : null
      ] })
    ] })
  ] });
}
...
function DateTimeSyncDialog({
  ...
  if (!open) return null;
  return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "settings-edit-backdrop", role: "presentation", onClick: (e) => e.target === e.currentTarget && !busy && onClose(), children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "settings-edit-dialog datetime-sync-dialog", role: "dialog", "aria-modal": "true", onClick: (e) => e.stopPropagation(), children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx("h2", { className: "settings-edit-title datetime-sync-title", children: "Date/Time Sync" }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "settings-edit-hint datetime-sync-subtitle", children: "Synchronize controller clock with current system time." }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "datetime-sync-time-card", children: /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "datetime-sync-time-value", children: displayValue }) }),
    error ? /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "settings-edit-error", role: "alert", children: error }) : null,
    success ? /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "settings-edit-success", role: "status", children: success }) : null,
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "settings-edit-actions", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("button", { type: "button", className: "settings-edit-btn settings-edit-btn--plain", disabled: busy, onClick: onClose, children: "Cancel" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        "button",
        {
          type: "button",
          ...
          children: busy ? "Synchronizing…" : "Synchronize Now"
        }
      )
    ] })
  ] }) });
...
function LoginRouteGuard() {
  return localStorage.getItem("PinSetup") === "true" ? /* @__PURE__ */ jsxRuntimeExports.jsx(Login, {}) : /* @__PURE__ */ jsxRuntimeExports.jsx(Navigate, { to: "/setup", replace: true });
}
...
function AppLayout() {
  ...
  const [lastSerialRxAt, setLastSerialRxAt] = reactExports.useState(null);
  ...
  const [dateTimeSyncOpen, setDateTimeSyncOpen] = reactExports.useState(false);
  const disconnectSerial = reactExports.useCallback(async () => {
    try {
  ...
  const clearSerialTraffic = reactExports.useCallback(() => {
    setSerialLines([]);
    serialLineKey.current = 0;
  }, []);
  const commHealthy = Boolean(serialSession) && lastSerialRxAt !== null && Date.now() - lastSerialRxAt <= 4e3;
  ...
  const controllerDateSpec = settingsConfig?.groups?.version_params?.["Controller's Date"] ?? monitorConfig?.groups?.version_params?.["Controller's Date"] ?? null;
  const timeSpec = settingsConfig?.groups?.version_params?.Time ?? monitorConfig?.groups?.version_params?.Time ?? null;
  ...
  reactExports.useEffect(() => {
    ...
    window.addEventListener("icms:traffic-line", onTrafficLine);
    return () => window.removeEventListener("icms:traffic-line", onTrafficLine);
  }, []);
  reactExports.useEffect(() => {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "app-layout", children: [
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "app-main", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("header", { className: "content-toolbar", children: [
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "content-toolbar-start", children: storedModel || storedFirmware ? /* @__PURE__ */ jsxRuntimeExports.jsxs(
          "p",
          {
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "content-toolbar-end", children: [
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsx(
            "button",
            {
              ...
              children: /* @__PURE__ */ jsxRuntimeExports.jsx("time", { className: "content-toolbar-datetime", dateTime: toolbarNow.toISOString(), children: toolbarNow.toLocaleString(void 0, {
                dateStyle: "medium",
                timeStyle: "medium"
              }) })
            }
      ...
      canOpenDateTimeSync && dateTimeSyncOpen ? /* @__PURE__ */ jsxRuntimeExports.jsx(
        DateTimeSyncDialog,
        {
          open: true,
          onClose: () => {
...
function App() {
  return /* @__PURE__ */ jsxRuntimeExports.jsx(BrowserRouter, { children: /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "app-shell", children: /* @__PURE__ */ jsxRuntimeExports.jsxs(Routes, { children: [
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsx(Route, { path: "/login", element: /* @__PURE__ */ jsxRuntimeExports.jsx(LoginRouteGuard, {}) }),
    /* @__PURE__ */ jsxRuntimeExports.jsx(Route, { path: "/setup", element: /* @__PURE__ */ jsxRuntimeExports.jsx(SetupRouteGuard, {}) }),
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs(Route, { element: /* @__PURE__ */ jsxRuntimeExports.jsx(AppLayout, {}), children: [
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx(Route, { path: "/auth", element: /* @__PURE__ */ jsxRuntimeExports.jsx(SetupRouteGuard, {}) })
    ] })
  ] }) }) });
}
